...
namespace BusinessLayer
{
    public class BANGLUONG
    {
        ...
        {
            List<tb_BANGLUONG> lst = db.tb_BANGLUONG.Where(x => x.MAKYCONG == makycong).ToList();
            db.tb_BANGLUONG.RemoveRange(lst);
            db.SaveChanges();
        }
        public void TinhLuongNhanVien(int makycong)
        {
            double luongngaythuong, luongphep,vang, luongtangca, luongchunhat, luongngayle, phucap, ungluong, thuclanh, hesoluong;
            ...
            foreach (var item in lstNV)
            {
                ...
                if (hd!=null)
                {
                    ...
                    vang = Convert.ToDouble(kcct.NGHIKHONGPHEP * luong1ngaycong);
                    luongphep = Convert.ToDouble(kcct.NGAYPHEP * luong1ngaycong * 0.5);
                    ...
                    luongtangca = Convert.ToDouble(db.tb_TANGCA.Where(x => x.MANV == item.MANV && (x.NAM * 100 + x.THANG)==makycong).Sum(x => x.SOTIEN));
