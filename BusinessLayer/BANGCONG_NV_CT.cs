...
namespace BusinessLayer
{
    public class BANGCONG_NV_CT
    {
        ...
        {
            return db.tb_BANGCONG_NV_CT.Where(x => x.MAKYCONG == makycong && x.MANV == manv && x.CONGNGAYLE != null).ToList().Sum(p => p.CONGNGAYLE.Value);
        }
        ...
        {
            return db.tb_BANGCONG_NV_CT.Where(x => x.MAKYCONG == makycong && x.MANV == manv && x.NGAYCONG != null).ToList().Sum(p => p.NGHIKHONGPHEP.Value);
        }
    }
}
